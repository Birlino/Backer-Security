    ...
    async execute(interaction){
        if(!interaction.isButton()) {return}
            ...
            if (interaction.customId == "admin") {
                ...
                .setDescription('Ticket aperto per **AMMINISTRAZIONE** un amministratore sarà da te a momenti perfavore attendi!')
                ...
                .addComponents(
                  ...
                  .setCustomId('close-ticket-admin')
                  .setLabel('close')
                  .setEmoji('✖') //Emoji del close
                  .setStyle('DANGER'))

                const opened = await c.send({
                    content: `<@&${config.amministratori}>`,
                    embeds: [embed],
                    components: [row]
                  });
            ...
            if (interaction.customId == "close-ticket-admin") {
            if(!interaction.member.roles.cache.has(config.staff)){
              return interaction.reply({content: `Non hai il ruolo `+ "<@&"+(config.staff)+">", ephemeral: true })
                ...
                const row = new Discord.MessageActionRow()
                  .addComponents(
                    new Discord.MessageButton()
                    .setCustomId('confirm-close-admin')
                    .setLabel('Close')
                    .setStyle('DANGER'),
                    new Discord.MessageButton()
                ...
                collector.on('collect', i => {
                  if (i.customId == 'confirm-close-admin') {
                    ...
                    chan.edit({
                      ...
                      .then(async () => {
                        ...
                        const row = new Discord.MessageActionRow()
                          .addComponents(
                            new Discord.MessageButton()
                            .setCustomId('delete-ticket-admin')
                            .setLabel('Delete')
              ...
              if (interaction.customId == "delete-ticket-admin") {
                const guild = client.guilds.cache.get(interaction.guildId);
                const chan = guild.channels.cache.get(interaction.channelId);
